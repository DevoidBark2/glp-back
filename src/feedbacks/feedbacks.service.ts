...
export class FeedbacksService {
	...
	async sendFeedBack(
		...
		const feedback = this.feedbackRepository.create({
			...
			message: message,
			recipient:
				user.role !== UserRole.SUPER_ADMIN ? superAdminUser : user
		})
	...
	async getFeedBackForUser(user: User) {
		...
		return await this.feedbackRepository.find({
			where: [
				...
				}, // Сообщения от данного пользователя к супер-администратору
				{
					sender: { id: superAdminUser.id },
					recipient: { id: user.id }
				} // Сообщения от супер-администратора к данному пользователю
			],
