...
export class PostService {
  ...
  async changePost(post: ChangePostDto,user: User) {
    const currentPost = await this.postEntityRepository.findOne({where: {
      id: post.id
    }})
    ...
    return {
      post: post,
      message: currentPost.status !== post.status && "Пост снят с публикации, так как статус изменился!"
    };
  }

  private async randomAssignPost(postId: number) {
    const moderators = await this.userRepository.find({where: {
