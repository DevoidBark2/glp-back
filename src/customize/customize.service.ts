...
export class CustomizeService {
    ...
    async selectedCustomizeItem(user: User, body: BuyItemDto) {
        ...
        switch (category) {
            case 'frames':
                purchase = await this.frameRepository.findOne({ where: { id: item.id } });
                break;
            case 'icons':
                purchase = await this.iconRepository.findOne({ where: { id: item.id } });
                break;
            case 'effects':
                purchase = await this.purchaseRepository.findOne({ where: { id: item.id } });
                break;
            default:
        ...
        let activeCustomization = await this.activeCustomizationRepository.findOne({
        ...
        if (!activeCustomization) {
            ...
            activeCustomization = this.activeCustomizationRepository.create({ user });
        }
        ...
        // Сохраняем изменения
        const updatedData = await this.activeCustomizationRepository.save(activeCustomization);
